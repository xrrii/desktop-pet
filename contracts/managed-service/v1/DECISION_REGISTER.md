...
### `D-P5-01` 管理员套餐分配与额度调整边界

状态：`Frozen`

- 只有带 `ROLE_ADMIN` 的官网 Session 可以读取活动套餐、分配用户套餐和调整能力额度；所有写请求必须携带当前 Session 的 CSRF Header，并记录不含敏感信息的安全审计。
- 分配套餐会取消目标用户现有活动订阅、创建新的有限周期订阅、删除旧能力授权，并按新套餐能力重建剩余额度；整个过程必须在单一数据库事务内完成。
- 额度调整使用显式增量语义，正数增加、负数减少，只允许对当前活动套餐中已启用的能力操作；结果必须保持在 0 到 2,000,000,000，溢出、负余额或无有效套餐时失败关闭。管理员赠送余额可以超过套餐默认月额度。
- 用户授权列表使用服务端分页，默认每页 20 条、最大 50 条；搜索条件、能力维度、页码和总数均由 PostgreSQL 权威查询返回，Web 不得先拉取全量用户再本地分页。
- 管理员可删除非管理员用户。删除必须在单一事务内清理 OAuth 授权、设备、Runtime Session、用量、订阅、Entitlement、偏好和试用领取事实，并撤销目标用户的 Web Session；删除试用领取事实不得回减当日已领取计数，避免通过删除账号返还或重复制造库存。
- 套餐分配和额度变更都必须统一递增目标用户全部 Entitlement Version；客户端需重新获取 Runtime Token，旧 Token 不得继续沿用旧权限。

### `D-P5-02` 官网 7 天试用套餐领取边界

状态：`Frozen`

- 试用套餐固定为 7 天，包含 Chat 100,000 tokens、Embedding 50,000 tokens、Rerank 50,000 tokens、Vision 100,000 tokens 和 Web Search 5 requests；套餐能力由 PostgreSQL 权威配置，Web 不得自造额度。
- 每个账号终身只能成功领取一次，领取事实独立于当前订阅永久保留；套餐到期、取消或被管理员替换后均不得再次领取。
- 只有没有生效套餐的活动账号可以领取，用户自助领取不得替换现有套餐。管理员分配与用户领取必须按账号串行并在单一事务中写入订阅、授权、库存和领取事实。
- 全站按 `Asia/Shanghai` 自然日默认最多成功发放 10 份。管理员只能以正增量增加当天总上限，不能减少上限、修改已领取数或影响后续日期；每日库存占用和上限追加必须由 PostgreSQL 对同一日期行原子更新，并发请求不得超发。
- Web 使用独立 Session 接口查询资格并通过带 CSRF 的显式 POST 领取。页面计数只用于展示，最终资格和库存始终以控制面为准。

### `D-ADMIN-01` 官网管理与访问统计专项

状态：`Frozen`（用户确认按默认方案开发；生产发布另行执行）

- 管理员统一管理用户授权、额度、套餐、删除和今日试用名额；新增状态筛选和用户详情，不启动收费、支付或服务端 Agent。
- 公开采集范围只有 `/`、`/share` 及首页两个 Releases 下载按钮；不采集来源地址，也不将运营统计关联账号。
- 北京时间、30 个自然日事实保留、30 天固定浏览器随机标识；DNT/GPC 不采集，存储不可用时 PV 可用、UV 不可去重。
- 仅采集 POST 精确端点匿名无状态、无凭据、无需 CSRF；管理查询与写入的现有 Session/管理员/CSRF 规则继续执行。
- PV、估算 UV、下载点击、同时间截止的昨日对比、日/小时趋势、固定枚举排行；未采集与部分覆盖不得伪装成完整零数据。
- 生产采集默认关闭，独立 Secret 注入；单实例内存限流默认单地址 120 次/分钟、全局 1200 次/分钟。这是初始保护上限，容量检查不代表生产请求承载承诺。

## Desktop SSO（2026-09-12）

用户确认：官网 A、桌面 B 后官网继续 A；每次选择账号，权限未变跳过重复 Consent。实现采用 300 秒事务、60 秒一次性凭证、32 字节随机标识，默认关闭独立开关；默认官网 origin https://petdock.site，生产强制固定。细节见 DESKTOP_SSO.md。
