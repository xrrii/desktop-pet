...
### `D-P5-01` 管理员套餐分配与额度增量边界

状态：`Frozen`

- 只有带 `ROLE_ADMIN` 的官网 Session 可以读取活动套餐、分配用户套餐和增加能力额度；所有写请求必须携带当前 Session 的 CSRF Header，并记录不含敏感信息的安全审计。
- 分配套餐会取消目标用户现有活动订阅、创建新的有限周期订阅、删除旧能力授权，并按新套餐能力重建剩余额度；整个过程必须在单一数据库事务内完成。
- 增加额度使用显式增量语义，只允许对当前活动套餐中已启用的能力操作；结果不得超过 2,000,000,000，溢出或无有效套餐时失败关闭。
