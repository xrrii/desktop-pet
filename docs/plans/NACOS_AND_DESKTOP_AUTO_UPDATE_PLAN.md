...
状态：Draft，需求与技术方案评审中；本文件不表示已开发或已部署。依据本次对话和当前仓库实现编写。已确认选择 Nacos 配置中心，以及桌面整体自动更新；不实施前端资源热更新。
...
| D7 | Nacos/SDK 版本与持久化 | 在 P0 核对支持矩阵、维护状态、授权许可和存储方案 | 技术选型待 P0；不得将“最新”直接写成生产版本 |
...
D1～D6 已获得用户回复，可据此准备 P0。签名方案和对象存储具体选型影响真实更新分发验收，但不阻塞 Nacos 原型；D7 的技术兼容性仍需实际验证。
...
| --- | --- | --- | --- |
| P0 选型/原型 | 版本矩阵、Nacos 单机配置、Java/Python 监听与故障样例、资源/恢复报告 | coordinator + release；完成后切换 cloud/qa | D1/D2/D6 明确；存储、权限、监听和资源通过 |
...
本次只新增方案与索引，未引入依赖、修改业务代码、安装 Nacos、创建更新源或触发 Jenkins。版本/持久化/加密兼容性尚未在线查证，不把方案推断写成已通过测试。

本次已回填 D1～D6 及统一全量发布的用户结论，下一步可形成 P0 任务单；Nacos 具体版本与持久化仍需验证，自动更新的签名方案和对象存储细节在 P3 前定稿。正式开发前，Cloud 内部配置 Schema 与任何新增公共更新协议各自确定权威位置；跨端协议仍由 Cloud 管理。阶段完成记录应包含实际提交、配置版本、客户端/Runtime 版本、使用的构建制品、测试命令、结果、未运行项和上线状态。
