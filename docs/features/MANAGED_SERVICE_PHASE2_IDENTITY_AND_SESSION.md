...
- 中国大陆云服务器已经就绪。
- `petdock.site` 域名已经就绪。
- ICP 备案、正式 DNS 和 TLS 条件已经就绪；生产隔离部署和来源 IP 白名单线上门禁尚未完成，普通用户流量未开放。
...
- 正式入口网关统一终止 TLS；生产数据、签名密钥、入口路由和来源 IP 白名单门禁通过前不开放面向普通用户的公网业务流量。
...
| `staging` | 当前固定正式端点的受限发布验收模式 | 独立测试数据 | 仅内部白名单；尚不能与 Production 长期并存 |
...
当前 Desktop/Cloud 的 `staging` Profile 与 `production` 一样固定官方端点，不是独立公网预发布环境。未来建立独立 Staging 必须先修改 Cloud 权威契约、Desktop 端点策略和 Cloud 生产校验，并独立配置数据库、Redis、OAuth 授权数据、JWT 密钥、日志和备份；不得只改 DNS 或运行时环境变量。
...
| 入口网关 | Nginx；正式部署按 Cloud 生产与线上联调指南接入 |
| 开发接入 | SSH 本地端口转发 |
...
云端已完成 P2-01、P2-02、P2-04、P2-05、P2-08、P2-09、P2-W01 和 P2-W02，具备持久化用户目录、设备、授权记录、Runtime Token、官网账号、Redis Web Session 和标准 Authorization Consent；Entitlement 管理与 Usage API 按确认延后。桌面端 P2-06 至 P2-10 已完成，`petdock-web` P2-W01/P2-W02 已通过真实 HTTPS Cookie/CSRF、分离 Host Session 和 Desktop loopback/PKCE 联调。
...
- 官网不可用、服务器不可达或 Managed 登录关闭时 BYOK 仍可用。
- Windows 开发版和解包版系统浏览器登录。
- 正式 DNS 和 TLS 下的来源 IP 白名单真实域名登录冒烟。
