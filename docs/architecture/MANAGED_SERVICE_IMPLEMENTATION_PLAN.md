...
1. 用户在独立部署的 PetDock 官网完成注册、登录、充值、套餐和用量管理，并通过系统浏览器授权桌面端。
...
- 首期在一台中国大陆云服务器上部署官网、入口网关、控制面、数据面及数据组件，不建设集群、跨可用区副本或自动容灾；各组件仍保持独立进程或容器和清晰职责边界。
...
Spring Boot 同时为官网前端提供账号、充值、套餐、订单、设备和用量看板 API，为 Electron 提供 OAuth、设备、Entitlement 和 Runtime Session API。
...
首期物理部署收敛到一台中国大陆云服务器：入口网关统一终止 TLS，官网、控制面、数据面、数据库和撤销存储以独立进程或容器运行，业务进程不直接暴露公网端口。第一版不部署应用集群、数据库集群、跨可用区副本、冷备服务器或自动故障转移。
...
- `P2-W03` 实现套餐、充值、订单、设备列表和用量看板的基础页面及路由；支付闭环可随 Phase 5 正式收费能力完善。
...
- `P5-10` 在独立官网完成真实充值、支付结果、套餐生效、账单、退款状态和生产级用量看板。
...
- 正式主域名为已购买的 `petdock.site`，分别使用根域名、`account`、`api` 和 `ai` 入口；DNS 解析和证书签发是公网灰度前置条件。
...
- 用户可以通过独立官网登录和管理充值、套餐、用量及设备；桌面端可以登录、退出、撤销当前设备并查看脱敏能力状态。
- 各能力可以从其允许的来源集合中独立选择。
